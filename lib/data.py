...
def get_loaders(name='wikitext2', nsamples=128, seed=0, seqlen=2048, tokenizer=None):
    ...
    dirname = 'tmp_files/'
    tkzrname = tokenizer.name.replace('/', '_')
    shortcut_file_prefix = f'{name}_{nsamples}_{seed}_{seqlen}_{tkzrname}'
    loader_path = os.path.join(dirname, f'{shortcut_file_prefix}_loader.pkl')
    enc_path = os.path.join(dirname, f'{shortcut_file_prefix}_enc.pkl')
    if os.path.exists(loader_path) and os.path.exists(enc_path):
        print('loading shortcut loader...')
        ...
        if 'wikitext2' in name:
            loader, enc = get_wikitext2(nsamples, seed, seqlen, tokenizer)
        elif "c4" in name:
            loader, enc = get_c4(nsamples, seed, seqlen, tokenizer)
        elif "ptb" in name:
            loader, enc = get_ptb(nsamples, seed, seqlen, tokenizer)
