...
class ActivationMeanHook:
    ...
    def __call__(self, module, inp, out):
        # TODO: ほんとはPADを無視したほうがいいかもしれないが、一旦PADも計算にいれてる
...
def prune_wandals(args, model, processor, device):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    ...
    with torch.no_grad():
        inps, outs, attention_mask, position_embeddings = prepare_calibration_text_input(model, dataloader, device)
    init_inps = inps.clone()
    init_outs = outs.clone()
    print("dataset loading complete")
    ...
    for i in tqdm(range(len(layers))):
        ...
        for name in subset:
            ...
            W_metric = (torch.abs(W) * x).sum(dim=0)
            params = {
                'num_heads': model.config.num_attention_heads, 
                'head_dim': model.config.head_dim,
                'pruning_ratio': args.pruning_ratio
            ...
            C = X_dash @ X_dash.T + (1e-4)*torch.eye(X_dash.shape[0], device=device, dtype=X_dash.dtype)
            W_opt = Y @ X_dash.T @ torch.linalg.inv(C)
            subset[name].weight.data[:, W_mask] = W_opt
...
def prune_wanda_rotation(args, model, processor, device):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
...
def prune_rcpu(args, model, processor, device):
    ...
    for i in tqdm(range(len(layers))):
        ...
        for name in subset:
            ...
            #if name != 'self_attn.o_proj': # アブレーション用
            # --------  プルーニング後 Procrustes -----------------
            W_p = W[:, keep]                         # [d_out, d']
            X_p = X[keep, :]                         # [d', N]
        ...
        for j in range(len(dataloader)):
            with torch.no_grad():
                outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask, position_embeddings=position_embeddings)[0]
        inps, outs = outs, inps
    torch.cuda.synchronize()             # 計測終了前に必ず同期
...
def prune_prop_ls(args, model, processor, device):
    ...
    for i in tqdm(range(len(layers))):
        ...
        for name in subset:
            ...
            X_keep = X[keep, :]
            C = X_keep @ X_keep.T + (1e-4)*torch.eye(X_keep.shape[0], device=device, dtype=X_keep.dtype)
            W_opt = Y @ X_keep.T @ torch.linalg.inv(C)
...
def prune_norot(args, model, processor, device):
    model.config.use_cache = False 
    print("loading calibdation data")
    dataloader, _ = get_loaders("wikitext2", nsamples=args.nsamples, seed=args.seed,
                                  seqlen=model.seqlen, tokenizer=processor)
    ...
    for i in tqdm(range(len(layers))):
        ...
        for name in subset:
            ...
            subset[name].weight.data[:, W_mask] = W_p
